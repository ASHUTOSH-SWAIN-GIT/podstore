...
import { Room, RoomEvent, Track, LocalTrackPublication, LocalParticipant, RemoteTrack, RemoteTrackPublication, RemoteParticipant, Participant } from 'livekit-client'
...
export const useSessionControls = () => {
  ...
  const handleParticipantDisconnected = (participant: RemoteParticipant) => {
    ...
    if (remoteVideosRef.current) {
      const participantDiv = remoteVideosRef.current.querySelector(`[data-participant-id="${participant.identity}"]`)
      if (participantDiv) {
  ...
  const handleTrackSubscribed = (
    ...
    if (track.kind === Track.Kind.Video || track.kind === Track.Kind.Audio) {
      ...
      if (remoteVideosRef.current) {
        ...
        let participantDiv = remoteVideosRef.current.querySelector(`[data-participant-id="${participant.identity}"]`) as HTMLDivElement
        
        if (!participantDiv) {
          // Create new participant div
          participantDiv = document.createElement('div')
          participantDiv.className = 'aspect-video bg-gray-900 rounded-lg border border-gray-800 relative overflow-hidden'
          participantDiv.dataset.participantId = participant.identity
          
          // Add participant name badge
          const badge = document.createElement('div')
          badge.className = 'absolute bottom-2 left-2 bg-black/50 text-white border-gray-600 text-xs px-2 py-1 rounded'
  ...
  const connectToRoom = async (sessionId: string, userId: string) => {
    ...
    try {
      ...
      const existingParticipants = Array.from(room.remoteParticipants.values()).length
      setLiveParticipantCount(existingParticipants)
      console.log(`Connected to room with ${existingParticipants} existing participants`)

      // Enable camera and microphone
      await room.localParticipant.setCameraEnabled(!isVideoOff, livekitConfig.videoOptions)
      await room.localParticipant.setMicrophoneEnabled(!isMuted, livekitConfig.audioOptions)
  ...
  const toggleMute = async () => {
    if (room) {
      ...
      await room.localParticipant.setMicrophoneEnabled(!newMutedState, livekitConfig.audioOptions)
  ...
  const toggleVideo = async () => {
    if (room) {
      ...
      await room.localParticipant.setCameraEnabled(!newVideoOffState, livekitConfig.videoOptions)
          ...
          room.localParticipant.videoTrackPublications.forEach((publication) => {
            if (publication.track && publication.kind === Track.Kind.Video) {
              ...
              if (localVideoRef.current) {
                localVideoRef.current.innerHTML = ''
                localVideoRef.current.appendChild(videoEl)
                console.log('Local video manually attached after enabling camera')
              }
            }
          })
