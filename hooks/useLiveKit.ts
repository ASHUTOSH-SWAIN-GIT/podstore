...
export const useLiveKit = () => {
  ...
  const connectToRoom = useCallback(async (url: string, token: string, selectedAudio: string, selectedVideo: string) => {
    if (!room || !token || !url) {
    ...
    setTimeout(() => {
      ...
      if (p.videoTrackPublications && p.videoTrackPublications.size > 0) {
        const videoPublication = Array.from(p.videoTrackPublications.values())[0] as LocalTrackPublication | undefined;
        ...
        console.error("No video track publications found. Retrying in 2 seconds...");
        setTimeout(() => {
          if (p.videoTrackPublications && p.videoTrackPublications.size > 0) {
            const videoPublication = Array.from(p.videoTrackPublications.values())[0] as LocalTrackPublication | undefined;
            const videoTrack = videoPublication?.track;
  ...
  }, [room, handleTrackSubscribed, handleTrackUnsubscribed, handleActiveSpeakerChange, handleDisconnect, handleLocalTrackUnpublished, handleLocalTrackPublished]);
